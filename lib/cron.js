  ...
  */
  ...
  _tick() {
    ...
    return this._model.findOneAndUpdate(
      {'cron.state': 0, 'cron.startAt': {$lte: tickDate}, $or: [{'cron.stopAt': {$gte: tickDate}}, {'cron.stopAt': {$exists: false}}]},
      {'cron.state': 1, 'cron.startedAt': tickDate},
      {sort: {'cron.startAt': 1}}
    ).then(res => {
      doc = res;
  ...
  _rescheduleDocument(doc) {
    ...
    if (!nextStart) {
      if (doc.cron.removeExpired === true) {
        ...
        return doc.update({'cron.state': 2, 'cron.processedAt': new Date()}); // mark as expired
      }
    } else {
      return doc.update({'cron.state': 0, 'cron.processedAt': new Date(), 'cron.startAt': nextStart}); // continue
    }
  }
