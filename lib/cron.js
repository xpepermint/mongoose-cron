  ...
  _rescheduleDocument(doc) {
    ...
    if (!nextStart) {
      if (doc.cron.removeExpired === true) {
        ...
        return doc.update({$unset: {'cron.locked': 1, 'cron.lastError': 1}, 'cron.processedAt': new Date()}); // mark as expired
      }
    } else {
    ...
    switch(err.name) {
      ...
      default:
        promise = promise.then(res => {
          return doc.update({$unset: {'cron.locked': 1, 'cron.enabled': 1}, 'cron.lastError': err.message});
        });
    }
